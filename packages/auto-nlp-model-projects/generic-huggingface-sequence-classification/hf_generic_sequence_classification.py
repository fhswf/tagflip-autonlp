...
class DataTrainingArguments:
    ...
    """
    task_name: Optional[str] = field(default="sc", metadata={"help": "The name of the task (ner, pos...)."})
    ...
    def train(self, datasets: DatasetDict, autonlp_args: AutoNLPArguments, args: Dict[ArgsType, Args],
        ...
        def compute_metrics(p):
            ...
            for i in range(len(metrics_list)):
                results = metric[i].compute(predictions=predictions, references=labels)
        ...
        max_test_samples = data_args.max_test_samples if data_args.max_test_samples is not None else len(valid_dataset)
        ...
        if hyperspace_tune_args:
            ...
            logger.info(f"Starting training with automatic hyperparameter search. There will be {hyperspace_tune_args.trials} trials.")
            ...
            model = AutoModelForSequenceClassification.from_pretrained(last_checkpoint_dir)
            tmp_trainer = Trainer(
                    model=model,
                    args=training_args,
                    train_dataset=train_dataset,
            ...
            tmp_trainer.add_callback(self.get_hf_tagflip_trainer_callback())  # this is important to attach tagflip capabilities
