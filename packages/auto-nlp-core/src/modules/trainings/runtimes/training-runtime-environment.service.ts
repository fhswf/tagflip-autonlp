...
@Injectable()
export class TrainingRuntimeEnvironmentService
  ...
  async getTypes(): Promise<string[]> {
    const response = await this.httpService
      .get<string[]>(`${this.deploymentServiceUrl}/training/runtime/types`)
      .toPromise();
  ...
  async findByName(name: string): Promise<RuntimeEnvironment> {
    const response = await this.httpService
      .get(`${this.deploymentServiceUrl}/training/runtime/${name}`)
      .toPromise();
  ...
  async findAll(): Promise<RuntimeEnvironment[]> {
    const response = await this.httpService
      .get<[]>(`${this.deploymentServiceUrl}/training/runtime`)
      .toPromise();
    return plainToClass(RuntimeEnvironment, response.data);
  }
  ...
  private async findAllByRuntimeType<T extends RuntimeEnvironment>(
    ...
    const response = await this.httpService
      .get<[]>(`${this.deploymentServiceUrl}/training/runtime`, { params })
      .toPromise();
